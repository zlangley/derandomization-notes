...
\begin{proofsk}
  The proof is by reconstruction: Assume contrapositively that there is some
  uniform distinguisher $D$ that is not fooled by the PRG. We will use $D$ to
  give a $\BPTIME\brac{2^{\eps n}}$ algorithm for $\fws$.
  ...
  \begin{enumerate}
    ...
    \item With high probability, $A$ returns a circuit that agrees with
      $\fws_i$ on a $1/2 + 1/n^2$ fraction of the inputs.
